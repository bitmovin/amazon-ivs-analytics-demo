...
import { fetchChannels, fetchStreamSessionDetails, fetchStreamSessionsForChannel, getChannel } from "./aws";
import { fetchInformation, fetchLicenses, fetchOrganizations } from "./bitmovin";
...
async function fetchOrgs() {
  ...
  const orgs = (response.items || []).flatMap(({ id, ...org }) => (id ? [{ ...org, id }] : []));
...
async function fetchOrgLicenses(org: Organization & { id: string }) {
  ...
  const licenses = items.flatMap(({ licenseKey, ...license }) => (licenseKey ? [{ ...license, licenseKey }] : []));
