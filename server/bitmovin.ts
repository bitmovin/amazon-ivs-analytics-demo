...
export type QueryParams<K extends QueryType> = Parameters<QueriesApi[K]["create"]>[0];
...
export const preloadInformation = (requestInit?: RequestInit, tenantOrgId?: string) => {
  void fetchInformation(requestInit, tenantOrgId);
};

export const fetchInformation = cache(async (requestInit?: RequestInit, tenantOrgId?: string) => {
  const client = getClient(requestInit, tenantOrgId);
  return await client.account.information.get();
});

export const preloadOrganizations = (requestInit?: RequestInit, tenantOrgId?: string) => {
  void fetchOrganizations(requestInit, tenantOrgId);
};

export const fetchOrganizations = cache(async (requestInit?: RequestInit, tenantOrgId?: string) => {
  const client = getClient(requestInit, tenantOrgId);
  return await client.account.organizations.list();
});

export const preloadOrganization = (organizationId: string, requestInit?: RequestInit, tenantOrgId?: string) => {
  void fetchOrganization(organizationId, requestInit, tenantOrgId);
};

export const fetchOrganization = cache(
  async (organizationId: string, requestInit?: RequestInit, tenantOrgId?: string) => {
    const client = getClient(requestInit, tenantOrgId);
...
export const fetchLicenses = cache(
  async (requestInit?: RequestInit, tenantOrgId?: string, params?: AnalyticsLicenseListQueryParams) => {
    const client = getClient(requestInit, tenantOrgId);
...
export const fetchImpression = cache(
  async (impressionId: string, licenseKey: string, requestInit?: RequestInit, tenantOrgId?: string) => {
    const client = getClient(requestInit, tenantOrgId);
...
export const fetchImpressions = cache(
  async (requestInit?: RequestInit, tenantOrgId?: string, searchParams?: AnalyticsImpressionsQuery) => {
    const client = getClient(requestInit, tenantOrgId);
    const response = await client.analytics.impressions.getImpressions(searchParams);
    return response;
  }
);
