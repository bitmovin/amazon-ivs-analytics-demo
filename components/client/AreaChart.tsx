...
const LazyAreaChart = lazy(() => import("@cloudscape-design/components/area-chart"));

if (typeof window === "undefined") {
  React.useLayoutEffect = React.useEffect;
}

export default function AreaChart({ fallback, ...props }: AreaChartProps<ChartDataTypes> & { fallback: JSX.Element }) {
  const series = props.series.map((series) =>
    series.type === "area" && props.xScaleType === "time" ? parseTimeSeries(series) : series
  );

  const xDomain =
    props.xDomain && props.xScaleType === "time" ? [new Date(props.xDomain[0]), new Date(props.xDomain[1])] : undefined;
