...
async function Component(params: { licenseKey?: string; orgId?: string; channelArn?: string }) {
  ...
  return (
    <ClientTopNavigation
      ...
      organizations={organizations.map(({ id, name = "[error]", licenses }) => ({
        id,
        name,
        licenses: licenses.map(({ name = "[error]", licenseKey = "[error]" }) => ({
          name,
          licenseKey,
        })),
      }))}
      channelName={channelArn ? channels.find(({ arn }) => arn === channelArn)?.name ?? "Channels" : "Channels"}
